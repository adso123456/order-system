...
**状态**：第一遍已完成 ✅
**创建于**：2026-06-19　**最后更新**：2026-06-20 15:15
...
- [x] **T15** Docker 容器化 — Dockerfile + docker-compose.yml ✅ 已完成
  - 文件：`Dockerfile`、`docker-compose.yml`、`.dockerignore`、`settings.xml`
  - 依赖：T14
  - 验证：`docker-compose up` 一键启动 app + MySQL + Redis；curl 接口正常返回；数据卷持久化通过

### 第二遍 · 分布式进阶

- [ ] **前置** Docker 数据迁盘 — 把 Docker 镜像/容器数据从 C 盘迁到 E 盘，释放 C 盘空间
  - 说明：当前 Docker Desktop 的镜像和卷存在 C 盘，mysql:8(1.12GB) + maven/temurin 等镜像较占空间，迁到 E 盘
  - 依赖：T15
  - 验证：docker images 镜像路径指向 E 盘；docker-compose up 正常启动
...
- **2026-06-20** — T15 Docker 容器化完成，第一遍收官 | 文件：Dockerfile(多阶段构建:maven:3.9-eclipse-temurin-17 → eclipse-temurin:17-jre)、docker-compose.yml(app+mysql:8+redis:7-alpine)、.dockerignore、settings.xml(阿里云 Maven 镜像)。要点：①application.yaml 参数化 DB/Redis 连接(环境变量注入，默认 localhost:3307/6379 保本地开发)；②Dockerfile 先拷 pom.xml 下依赖(层缓存)再编译，settings.xml 走阿里云镜像避免容器内从外网下依赖超时；③compose MySQL healthcheck(mysqladmin ping) + app depends_on service_healthy，避免应用先启连不上库崩溃；④mysql_data 卷持久化，down 再 up 数据不丢；⑤compose 内用服务名 mysql:3306/redis:6379 互联。🪤 踩坑：①第一版写 mysql:8.0 但本地已有 mysql:8(1.12GB)，改 tag 复用省下载；②8080 端口被残留 mvnw 进程占用，kill 后重来。**第一遍 15 个任务全部完成** | 下一步：歇一歇，消化吸收，第二遍前写 README.md 面试门面
